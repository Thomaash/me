  ...
  <v-form v-model="valid">
    <v-container grid-list-md>
      <v-layout wrap>
        ...
        <v-flex xs12 md9>
          <v-text-field ref="itemSTPPriority" v-model.number="item.stpPriority" :rules="[badNumberRule('itemSTPPriority')]" :error-messages="errors.item.stpPriority" label="STP Priority" type="number" step="4096" min="0" max="65535" clearable data-cy="edit-stp-priority" />
        </v-flex>
        <v-flex xs12>
          <v-text-field v-model="item.ip" :error-messages="errors.item.ip" label="IP" clearable data-cy="edit-ip" />
        </v-flex>
        <v-flex xs12>
          <v-text-field ref="itemDPCTLPort" v-model.number="item.dpctlPort" :rules="[badNumberRule('itemDPCTLPort')]" :error-messages="errors.item.dpctlPort" label="DPCTL Port" type="number" min="1" max="65535" clearable data-cy="edit-dpctl-port" />
        </v-flex>
        ...
        <v-flex xs12>
          <v-text-field v-model="item.dpopts" label="Ofdatapath arguments" clearable data-cy="edit-dpopts" />
        </v-flex>
        <v-flex xs12>
          <v-text-field ref="itemReconnectMs" v-model.number="item.reconnectms" :rules="[badNumberRule('itemReconnectMs')]" :error-messages="errors.item.reconnectms" label="Reconnect Timeout" type="number" min="0" suffix="ms" clearable data-cy="edit-reconnect-ms" />
        </v-flex>
        ...
        <v-flex xs12>
          <ThreeStateCheckbox v-model="item.inNamespace" label="In Namespace" data-cy="edit-in-namespace" />
        </v-flex>
        <v-flex xs12>
          <ThreeStateCheckbox v-model="item.batch" label="Batch" data-cy="edit-batch" />
        </v-flex>
        <v-flex xs12>
          <ThreeStateCheckbox v-model="item.verbose" label="Verbose" data-cy="edit-verbose" />
        </v-flex>
        <v-flex xs12>
          <v-text-field v-model="item.opts" label="Additional Switch Options" clearable data-cy="edit-opts" />
        </v-flex>
        <v-flex xs12>
