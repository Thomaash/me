  ...
  methods: {
    ...
    async handleSave() {
      ...
      try {
        ...
        const res = await fetch( import.meta.env.VITE_API_URL + "/api/configs", {
          method: "POST",
          headers: {
    ...
    async handleLoadConfig(configName) {
      try {
        ...
        const res = await fetch(import.meta.env.VITE_API_URL + `/api/configs/${encodeURIComponent(configName)}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
