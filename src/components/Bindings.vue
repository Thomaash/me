<template>
  <section>
    ...
    <v-data-table :headers="headers" :items="items" item-key="combination" :items-per-page="items.length" :sort-by="[]" hide-default-footer class="elevation-1">
      <template #item.combination="{ item }">
        <div class="with-title combination" :title="getTitleFor(item.combination)">
          <span
            :is="c.tag"
...
export default {
  ...
  methods: {
    ...
    getTitleFor (str) {
      return this.parse(str).map(c => c.title).join(' ')
    },
    update () {
      ++this.iconsIndex
