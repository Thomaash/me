...
const generators = {
  ...
  link (item) {
    const parts = [
      ...(item.bandwidth != null ? [`<tr><td>Bandwidth</td><td>${item.bandwidth} MBit/s</td></tr>`] : []),
      ...(item.delay != null ? [`<tr><td>Delay</td><td>${fixUnit(item.delay)}</td></tr>`] : []),
      ...(item.loss != null ? [`<tr><td>Loss</td><td>${item.loss} %</td></tr>`] : []),
      ...(item.maxQueueSize != null ? [`<tr><td>Max Queue</td><td>${item.maxQueueSize} packets</td></tr>`] : []),
      ...(item.jitter != null ? [`<tr><td>Jitter</td><td>${fixUnit(item.jitter)}</td></tr>`] : [])
    ]
