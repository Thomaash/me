...
function buildOutputString (items, left, right) {
  if (items.length === 0) {
    ...
    const indent = items.reduce((acc, item) => Math.max(item[left].length, acc), 0) + 2
    const sepparator = items.reduce((acc, item) => Math.max(item[right].length, acc), 0) > 1 ? '\n\n' : '\n'
    return items
      .map(item => `${(item[left] + ':').padEnd(indent, ' ')}${item[right].join('\n'.padEnd(indent + 1, ' '))}`)
      .join(sepparator)
  }
}
