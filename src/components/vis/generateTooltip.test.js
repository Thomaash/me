...
describe.concurrent("generateTooltip", () => {
  describe("dispatch", () => {
    it("delegates to the correct generator based on item.type", ({ expect }) => {
      expect(generateTooltip({ type: "host", defaultRoute: "10.0.0.1" }))
        .toBe("Default Route: 10.0.0.1");
    });
    ...
    it.each([
      {
        props: { bandwidth: 100 },
        desc: "bandwidth",
        expected: "<table><tr><td>Bandwidth</td><td>100 MBit/s</td></tr></table>",
      },
      {
        props: { delay: "10ms" },
        desc: "delay",
      ...
      },
      {
        ...
        desc: "maxQueueSize",
        expected: "<table><tr><td>Max Queue</td><td>500 packets</td></tr></table>",
      },
      {
        props: { jitter: "2ms" },
        desc: "jitter",
  ...
  describe("host generator", () => {
    it("returns 'Default Route: <route>' when defaultRoute is set", ({ expect }) => {
      expect(generateTooltip({ type: "host", defaultRoute: "192.168.1.1" }))
        .toBe("Default Route: 192.168.1.1");
    });

    it("returns 'No default route' when defaultRoute is absent", ({ expect }) => {
  ...
  describe("switch generator", () => {
    ...
    it("returns 'Default' when switchType is null", ({ expect }) => {
      expect(generateTooltip({ type: "switch", switchType: null }))
        .toBe("Default");
    });

    it("returns the raw switchType value when it is not in switchTypesMap", ({ expect }) => {
      expect(generateTooltip({ type: "switch", switchType: "CustomSwitch" }))
        .toBe("CustomSwitch");
    });
  });
  ...
  describe("controller generator", () => {
    ...
    it("shows 'Default' for controllerType when controllerType is absent", ({ expect }) => {
      const result = generateTooltip({
        type: "controller",
        ip: "10.0.0.1",
        port: 8080,
    ...
    it("returns the raw controllerType when not in controllerTypesMap", ({ expect }) => {
      const result = generateTooltip({
        type: "controller",
    ...
    it("shows only 'Default<br/>' when neither ip nor port is set", ({ expect }) => {
      const result = generateTooltip({ type: "controller" });
      expect(result).toBe("Default<br/>");
    });

    it("shows controllerType and '<br/>' only when both ip and port are null", ({ expect }) => {
      const result = generateTooltip({
        type: "controller",
        controllerType: "Ryu",
        ip: null,
    ...
    it("shows ip only (no colon) when ip is present and port is explicitly null", ({ expect }) => {
      const result = generateTooltip({
        type: "controller",
        controllerType: "NOX",
        ip: "192.168.1.1",
    ...
    it("shows '<No IP>:port' when ip is explicitly null and port is present", ({ expect }) => {
      const result = generateTooltip({
        type: "controller",
  ...
  describe("port generator edge cases", () => {
    it("shows 'Physical interface' and 'No addresses' when physical is true but ips is absent", ({ expect }) => {
      const result = generateTooltip({ type: "port", physical: true });
      expect(result).toBe("Physical interface<br/>No addresses");
    });

    it("shows only 'No addresses' when both physical and ips are absent", ({ expect }) => {
