...
export default class {
  ...
  build () {
    ...
    items.arr.port.filter(port => port.ips && port.ips.length).forEach(port => {
      const node = this._portToNode(port)
      const nodeHostname = node
        ? node.hostname
        : ''
      ...
      const planPort = planNode.ports[port.hostname] = planNode.ports[port.hostname] || []
  ...
  savePDF (headline, filename) {
    ...
    Object.entries(this.plan).sort(compareEntries).forEach(([nodeHostname, node]) => {
      ...
      Object.entries(node.ports).sort(compareEntries).forEach(([portHostname, ips]) => {
        let firstInPort = true
        ips.forEach(ip => {
          const row = []

          if (firstInNode) {
            row.push({ rowSpan: node.length, content: nodeHostname, styles: { valign: 'middle' } })
          }
          if (firstInPort) {
            row.push({ rowSpan: ips.length, content: portHostname, styles: { valign: 'middle' } })
          }
          body.push([...row, ip])
