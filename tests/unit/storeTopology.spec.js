  ...
  describe('Getters', () => {
    ...
    describe('Bounding box', () => {
      ...
      it('With more margin', () => {
        ...
        expect(getters.boundingBox(state)({ margin: 243 }))
          .to.deep.equal({
            eX: 1591,
            eY: 608,
            empty: false,
      ...
      it('With scale', () => {
        ...
        expect(getters.boundingBox(state)({ scale: Math.PI }))
          .to.deep.equal({
            eX: 4550,
            eY: 1461,
            empty: false,
      ...
      it('With scale and more margin', () => {
        ...
        expect(getters.boundingBox(state)({ scale: Math.PI, margin: 174 }))
          .to.deep.equal({
            eX: 4782,
            eY: 1694,
            empty: false,
  ...
  describe('Mutations', () => {
    describe('Import data', () => {
      function testImportedProject (state, externalData) {
        ...
        expect(Object.values(state.data.items))
          .to.deep.have.members(externalData.items)
      }
    ...
    describe('Undo/redo shifts', () => {
      ;[{
        ...
        after: { past: MAX_UNDO_LENGTH / 2 - 1, future: MAX_UNDO_LENGTH / 2 + 1 }
      }, {
        operation: 'undoShift',
        ...
        it(`Operation: ${operation}, past: ${before.past}, future: ${before.future}`, () => {
          ...
          if (operation === 'undoShift' && before.past) {
            if (before.future) {
              expect(state.future)
                .to.include(unitFuture, 'Future unit should be in the future')
            }
            if (before.past) {
              expect(state.future)
                .to.include(unitPast, 'Past unit should be in the future')
            }
          } else if (operation === 'redoShift') {
            if (before.future) {
              expect(state.past)
                .to.include(unitFuture, 'Future unit should be in the past')
            }
            if (before.past) {
              expect(state.past)
                .to.include(unitPast, 'Past unit should be in the past')
            }
          }
        })
      })
